...
export function quote({ meal, guests, date }) {
  ...
  if (pair) {
    ...
    const b = g * single
    if (a <= b) {
      if (pairs) lines.push({ label: `${SERVICE[meal].label} 1+1 · two guests`, qty: pairs, each: pair, amount: pairs * pair, deal: true })
...
export function headlineDeals() {
  ...
  return [
    mk('lunch-1-plus-1',  'lunch',  'pair',   'weekday', 'weekday', 'Monday to Friday · two guests'),
    mk('dinner-1-plus-1', 'dinner', 'pair',   'weekday', 'weekday', 'Monday to Friday · two guests'),
    ...
    mk('weekend-lunch-2', 'lunch',  'pair',   'weekend', 'weekend', 'Saturday & Sunday · two guests'),
    mk('weekend-dinner-2','dinner', 'pair',   'weekend', 'weekend', 'Saturday & Sunday · two guests'),
  ]
}
