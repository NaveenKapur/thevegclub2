/*  CRM adapter — RadissonVeg CRM (cis on pct211, public https://crm.thevegclub.com)
 *
...
/** A guest's coupon, for the public /c/[code] page. Server-side like everything
 *  else here: the browser never learns the CRM address, and the CRM stays the
 *  only authority on whether a coupon is real or already spent. */
export async function getPublicCoupon(code) {
  const r = await call(`${CRM.publicCouponPath}/${encodeURIComponent(code)}`)
  if (!r.ok) return { ok: false, error: r.error, status: r.status }
  return { ok: true, data: r.data || {} }
}

/** The QR image the CRM renders. Referenced directly by <img>, so the rotating
 *  token never has to travel through this site as text. */
export function couponQrUrl(code) {
  return `${CRM.base}${CRM.publicCouponPath}/${encodeURIComponent(code)}/qr`
}
